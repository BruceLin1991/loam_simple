...
LaserOdometry::LaserOdometry(ScanRegistration*scanRegsitrationObj)
...
{
    scanRegsitrationObject=scanRegsitrationObj;

    // advertise laser odometry topics
   /* ros::NodeHandle node;
    _pubLaserCloudCornerLast = node.advertise<sensor_msgs::PointCloud2>("/laser_cloud_corner_last22", 2);
    _pubLaserCloudSurfLast   = node.advertise<sensor_msgs::PointCloud2>("/laser_cloud_surf_last22", 2);
    _pubLaserCloudFullRes    = node.advertise<sensor_msgs::PointCloud2>("/velodyne_cloud_322", 2);
    _pubLaserOdometry        = node.advertise<nav_msgs::Odometry>("/laser_odom_to_init", 5);*/


    //open thread to get data from ScanRegistration 
    getPointCloudContinue=true;
    std::thread t1(&LaserOdometry::getPointCloudFromScanRegisThd,this);
    t1.detach();
}
...
bool LaserOdometry::getPointCloudAndInitOdom(nav_msgs::Odometry&_laserOdometryMsg,
...
{
    if(stackPointCloudTotal.empty()){
        return false;
    }
    point_cloud_lock.lock();
    OdometryCloudTotal unit = stackPointCloudTotal.top();
    stackPointCloudTotal.pop();
    point_cloud_lock.unlock();
    _laserOdometryMsg=unit._laserOdometryMsg;

    //judge if timestamp is continue
    ros::Time currentTimeStamp=_laserOdometryMsg.header.stamp;
    long long tmpTimeStamp=(long long)currentTimeStamp.sec*1000+currentTimeStamp.nsec/1000000;
    //ROS_INFO_STREAM("timestamp: " << currentTimeStamp<<"  "<<tmpTimeStamp); 
    if(tmpTimeStamp<preTimeStamp){
        cout<<"LaserOdometry.cpp：*****************timestamp is not continue******************"<<endl;
        cout<<"LaserOdometry.cpp：tmpTimeStamp and preTimeStamp is "<<tmpTimeStamp<<"  "<<preTimeStamp<<endl;
        return false;
    }
    preTimeStamp=tmpTimeStamp;
    pcl::copyPointCloud(unit._lastCornerCloud,_lastCornerCloud);
    pcl::copyPointCloud(unit._lastSurfaceCloud,_lastSurfaceCloud);
    pcl::copyPointCloud(unit._laserCloud,_laserCloud);

    return true;
}

void LaserOdometry::getPointCloudFromScanRegisThd()
{
    while(getPointCloudContinue)
    {
        ros::Time pointCloudTimeStamp;
        ...
        bool isOK=scanRegsitrationObject->getPointCloud(pointCloudTimeStamp,cornerPointsSharp,cornerPointsLessSharp,surfPointsFlat,surfPointsLessFlat,laserCloud);
        if(!isOK){
            usleep(1000*50);
            continue;
        }
        _cornerPointsSharp=cornerPointsSharp.makeShared();
        _cornerPointsLessSharp=cornerPointsLessSharp.makeShared();
        _surfPointsFlat=surfPointsFlat.makeShared();
        _surfPointsLessFlat=surfPointsLessFlat.makeShared();
        _laserCloud=laserCloud.makeShared();
        ...
        _laserOdometryMsg.header.stamp = pointCloudTimeStamp;
        ...
        OdometryCloudTotal tmpPointCloudTotal;
        pcl::copyPointCloud(*_lastCornerCloud, tmpPointCloudTotal._lastCornerCloud);
        pcl::copyPointCloud(*_lastSurfaceCloud, tmpPointCloudTotal._lastSurfaceCloud);
        pcl::copyPointCloud(*_laserCloud, tmpPointCloudTotal._laserCloud);
        tmpPointCloudTotal._laserOdometryMsg=_laserOdometryMsg;
        point_cloud_lock.lock();
        if(stackPointCloudTotal.size()>0)
        {           
            std::stack<OdometryCloudTotal>empty;
            std::swap(stackPointCloudTotal,empty);
        }
        stackPointCloudTotal.push(tmpPointCloudTotal); //Segmentation fault (core dumped)
        point_cloud_lock.unlock();
        ROS_INFO_STREAM("LaserOdometry.cpp:stackPointCloudTotal.size(): "<<stackPointCloudTotal.size());

        //publish
        /*ros::Time sweepTime = pointCloudTimeStamp;
...
void LaserOdometry::process()
{
    ...
    Vector3 v( _transform.pos.x() , _transform.pos.y()  ,_transform.pos.z() * 1.05  );
...
size_t LaserOdometry::transformToEnd(pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud)
{
    ...
    for (size_t i = 0; i < cloudSize; i++) {
        ...
        point.x += _transform.pos.x() ;
        point.y += _transform.pos.y() ;
        point.z += _transform.pos.z() ;
    }

    return cloudSize;
}
void LaserOdometry::closeThread()
{
